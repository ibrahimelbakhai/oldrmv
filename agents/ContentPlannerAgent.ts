...
export class ContentPlannerAgent extends BaseAgent {
  constructor(definition: AgentDefinition, manifestEntry: AgentManifestEntry) {
    ...
    if (!this.agentDefinition.steps || this.agentDefinition.steps.length === 0) {
      console.warn(`${this.manifestEntry.className} (ID: ${this.agentDefinition.id}) has no steps defined. Using fallback logic or expect errors if steps are required.`);
    }
  }
  ...
  public async invokeAction(actionName: string, params: any, callingAgentId?: string, planId?: string): Promise<LLMServiceResponse | any> {
    ...
    // Default action "generateOutline" might expect 'topic' in params
    if (actionName === 'generateOutline' && (!params || typeof params.topic !== 'string')) {
      return { text: null, error: "Missing or invalid 'topic' parameter for generateOutline action." };
    }
